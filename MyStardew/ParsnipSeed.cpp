...
void CParsnipSeed::Initialize(void)
{
	m_tInfo.fCX = TILECX;
	m_tInfo.fCY = TILECY;
	m_iNum = 1;

	m_iItemID = PARSNIPSEED;

	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/Obj/ParsnipSeed.bmp", L"ParsnipSeed");

	Update_Rect();
}

int CParsnipSeed::Update(void)
{
	Update_Rect();
...
int CParsnipSeed::Late_Update(void)
{
	return 0;
}

void CParsnipSeed::Render(HDC hDC)
{
	HDC	hMemDC = CBmpMgr::Get_Instance()->Find_Image(L"ParsnipSeed");
...
void CParsnipSeed::Release(void)
{
}
