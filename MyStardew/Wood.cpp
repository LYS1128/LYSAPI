...
void CWood::Initialize(void)
{
	...
	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/Obj/Wood.bmp", L"Wood");
...
{
		HDC	hMemDC = CBmpMgr::Get_Instance()->Find_Image(L"Wood");
		...
		GdiTransparentBlt(hDC,			   // 복사받을 dc, 최종적으로 그림을 그릴 dc
			int(m_tRect.left + CScrollMgr::Get_Instance()->Get_ScrollX()*m_Seed),  // 복사 받을 위치 x좌표
			int(m_tRect.top + CScrollMgr::Get_Instance()->Get_ScrollY()*m_Seed),				   // 복사 받을 위치 y좌표
